...
enforcement body has jurisdiction. Entirely passive — no interaction with the
target was permitted, or needed.
...
**Pinned is a curated shop window, not an inventory.** Everything the task
actually wanted was in the four I could not see.
...
answers are worthless in an investigation — if a client asks how you know, "a
model told me" does not survive the question — so I re-derived both from the
artefacts and changed how I search.
...
gives the UID, plus creation and expiry dates — and the creation date here
matched the day of the intrusion, corroborating the timeline from a completely
separate artefact.
...
forever — in the web UI, no clone required: repository → commits → read the
messages before the diffs, because people write things like "Update
...
like any domain — the registrable domain names the organisation, the subdomain
is just which regional server they connected to.
...
load-bearing — blossom in full bloom in January is itself a geographic
constraint.
...
answered a question in the following section — the home city — which is the
synthesis the room was actually teaching.
...
```mermaid
flowchart TD
    A["SVG left at scene"] --> B["Editor metadata: /home path"]
    B --> C["OS username"]
    C --> D["Code-hosting profile"]
    D --> E["Unpinned repos: deleted commit"]
    D --> F["PGP key: self-asserted name + email"]
    E --> G["Mining config: wallet + pool"]
    G --> H["Block explorer: payouts, exchange cash-out"]
    F --> I["Secondary social persona"]
    I --> J["Travel photos: route home"]
    I --> K["Saved network list: SSID history"]
    K --> L["Wardriving DB: BSSID + coordinates"]
    J --> M["Home city"]
    L --> M
    M --> N["Referral to suspect's national jurisdiction"]
```
...
make it a build step, not a habit. Treat a committed secret as burned and
...
an SSID containing a person, family or company name is a public, permanent
geolocation beacon in databases nobody thinks to check.
